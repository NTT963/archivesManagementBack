$(function () {
    ...
    function codeSuccess(data) {
        ...
        if(data.validate == "success"){
            ...
            //调用方法
            try {
                callAxiosPost("/archivesManagementBack/staff/login.do", data, loginSuccess, loginFail);
            } catch (e) {
                console.log(e);
            }
        }
        ...
        if(data.validate=="fail"){
            //验证码错误,刷新页面，选择性重置相关值
            vLogin.loginMessage='验证码错误，请重新输入'
            refresh();
            return ;
        }
        ...
        if(vLogin.userName==null||vLogin.userName==''){
            vLogin.loginMessage='请输入用户名！'
            refresh(); //刷新验证码
            return ;
        }

        //2.2 秘密不能为空
        if(vLogin.password==null||vLogin.password==''){
            vLogin.loginMessage='请输入密码！'
            refresh(); //刷新验证码
            return ;
        }
    ...
    function loginSuccess(data){
        ...
        if(data.loginStatus=="success"){
            ...
            console.log("登录成功")
            console.log(data)
            window.location.href="page/UserPage.html"
        }else if(data.loginStatus=="fail"){
            //登录失败，显示失败信息并重置参数
            // resetParams(data.message);
        }
    }
    ...
    function sendBtn() {
        if (tab == 'account_number') {
            $(".log-btn").click(function () {
                // var type = 'phone';
                var url = "/archivesManagementBack/staff/compareYzm.do";
                //设置方法调用参数
                var data = {
                    picCode: $.trim($('#veri').val())
                };
                console.log(JSON.stringify(data))
                console.log("调用接口")
                callAxiosPost(url, data, codeSuccess, codeFail);
...
// 						ldata.code = code;
// 					}
// 					console.log("参数" + JSON.stringify(ldata))
// 					// window.location.href = "AdminPage.html";
// //					window.navigate("AdminPage.html");
// 					$.ajax({
// 			            url: '/archivesManagementBack/staff/compareYzm.do',
// 			            type: 'post',
// 			            dataType: 'json',
// 			            async: true,
// 			            data: ldata,
